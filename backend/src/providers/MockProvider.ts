...
const CLAIM_KEYWORDS = [
  ...
  // Returns & disputes
  "отказ", "отменить", "аннулир", "спор", "арбитраж", "медиац",
  "оспорить", "не согласен", "не согласна", "апелляц",
  // Account & store blocks
  "заблокир", "блокировк", "ограничен", "приостановл", "деактив",
  "бан", "забанили", "разблокир", "восстановить аккаунт",
  "нарушение правил", "санкции",
  // Escalation markers
  "последний раз", "в третий раз", "который раз", "игнорируете",
  "обратился повторно", "повторная",
];
...
const CLAIM_CATEGORIES: Record<ClaimCategory, string[]> = {
  ...
  return_demand: ["возврат", "вернуть", "верните", "возврат денег", "вернуть деньги", "отказ от товар"],
  ...
  store_block: ["заблокир", "блокировк", "забанили", "приостановл", "деактив", "разблокир", "нарушение правил", "ограничен аккаунт"],
  other: [],
};
...
const TRACKING_REGEX = /(?:трек[а-яё]*[-\s]*(?:номер|код)?[-\s:]*)([A-Z]{2}\d{9}[A-Z]{2}|\d{10,20})/i;
const SKU_REGEX = /(?:артикул[а-яё]*|SKU|арт\.?)[\s:]*([A-Za-zА-Яа-я0-9\-]{4,20})/i;
...
const CLAIM_REPLIES: Record<ClaimCategory, { reply: string; formalReply: string; docs: string[] }> = {
  ...
  store_block: {
    reply: "Здравствуйте! Блокировка магазина/аккаунта на маркетплейсе — серьёзная ситуация. Рекомендуем: 1) Проверить уведомления в личном кабинете — там указана причина блокировки. 2) Подать апелляцию через форму маркетплейса с описанием ситуации и подтверждающими документами. 3) При необоснованной блокировке направить досудебную претензию в адрес маркетплейса (ст. 10 ГК РФ — запрет злоупотребления правом). Если нужна помощь с составлением апелляции — опишите причину блокировки подробнее.",
    formalReply: "Уважаемый продавец!\n\nПо вопросу блокировки Вашего аккаунта/магазина на маркетплейсе сообщаем следующее.\n\nРекомендуемый порядок действий:\n\n1. Ознакомьтесь с причиной блокировки в уведомлении маркетплейса.\n2. Подготовьте апелляцию с указанием:\n   — обстоятельств ситуации;\n   — подтверждающих документов (сертификаты, декларации, лицензии);\n   — обоснования отсутствия нарушений.\n3. Подайте апелляцию через официальную форму маркетплейса.\n\nПравовые основания для защиты:\n— Ст. 10 ГК РФ: запрет злоупотребления правом.\n— Ст. 15 ГК РФ: возмещение убытков, причинённых необоснованной блокировкой.\n— ФЗ-135 «О защите конкуренции»: если блокировка носит дискриминационный характер.\n— ФЗ-229 «Об исполнительном производстве»: если блокировка связана с исполнительным производством.\n\nПри отказе в разблокировке рекомендуем обратиться с жалобой в ФАС или направить досудебную претензию.\n\nС уважением,\nСлужба правовой поддержки",
    docs: ["Скриншот уведомления о блокировке", "Номер аккаунта/магазина", "Документы на товар (сертификаты, декларации)", "Переписка с поддержкой маркетплейса"],
  },
  other: {
...
export class MockProvider implements AIProvider {
  ...
  private extractFacts(message: string): ExtractedFacts {
    ...
    // Tracking number
    const trackMatch = message.match(TRACKING_REGEX);
    if (trackMatch) facts.tracking_number = trackMatch[1];

    // SKU / article
    const skuMatch = message.match(SKU_REGEX);
    if (skuMatch) facts.sku = skuMatch[1];
    ...
    else if (matchesAny(message, ["заблокир", "блокировк", "забанили"])) facts.problem = "Блокировка магазина/аккаунта";
    ...
    else if (matchesAny(message, ["разблокир", "восстановить"])) facts.customer_demand = "Разблокировка аккаунта";
    ...
    if (matchesAny(message, ["фас", "антимонопольн"])) markers.push("ФАС / антимонопольная служба");
  ...
  private computeDeadline(category: ClaimCategory): string {
    switch (category) {
      case "return_demand":
        return "10 дней на возврат денежных средств (ст. 22 ФЗ-2300-1 «О защите прав потребителей»)";
      case "court_threat":
        return "10 дней на рассмотрение претензии (ст. 22 ФЗ-2300-1)";
      case "defect":
      case "malfunction":
        return "20 дней на проверку качества / 10 дней на возврат (ст. 21, 22 ФЗ-2300-1)";
      case "mismatch":
        return "7 дней на возврат при дистанционной покупке (ст. 26.1 ФЗ-2300-1)";
      case "delivery_delay":
        return "Новый срок по соглашению сторон (ст. 23.1 ФЗ-2300-1)";
      case "incomplete":
        return "10 дней на доукомплектацию (ст. 22 ФЗ-2300-1)";
      case "store_block":
        return "30 дней на рассмотрение апелляции маркетплейсом (регламент площадки)";
      default:
        return "10 дней (ст. 22 ФЗ-2300-1)";
    }
  }

  private computeFinancialRisk(riskLevel: RiskLevel, category: ClaimCategory): string {
    if (category === "store_block") {
      return "Упущенная выгода за период блокировки (ст. 15 ГК РФ) + возможные убытки";
    }
    if (riskLevel === "high") {
      return "Цена товара + штраф 50% (п.6 ст.13 ФЗ-2300-1) + неустойка 1%/день (ст. 23 ФЗ-2300-1) + моральный вред (ст. 15 ФЗ-2300-1)";
    }
    if (riskLevel === "medium") {
      if (category === "return_demand") {
        return "Цена товара + возможная неустойка 1%/день при просрочке возврата (ст. 23 ФЗ-2300-1)";
      }
  ...
  private computeLegalActions(category: ClaimCategory, riskLevel: RiskLevel): string[] {
    ...
    if (category === "store_block") {
      actions.push("Проверить причину блокировки в уведомлении маркетплейса");
      actions.push("Подготовить апелляцию с подтверждающими документами");
      actions.push("При необоснованной блокировке — направить досудебную претензию (ст. 10, 15 ГК РФ)");
      actions.push("Рассмотреть обращение в ФАС при дискриминационных условиях (ФЗ-135)");
    }
...
const CLAIM_CATEGORY_LABELS_INTERNAL: Record<string, string> = {
  ...
  store_block: "Блокировка магазина",
  other: "Другое",
};
